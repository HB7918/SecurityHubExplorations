...
export default function FindingsList({ onSelectFinding, filterType }: FindingsListProps) {
  ...
  const filteredByTab = mockFindings.filter(f => {
    ...
    if (selectedTab === 'exposures') return f.type === 'Exposure';
    if (selectedTab === 'threats') return f.type === 'Threat';
    return true;
  });
  ...
  return (
    <Container header={<div style={{ fontSize: 16, fontWeight: 700 }}>Priority hub <span style={{ fontWeight: 400, color: "#5f6b7a" }}>({sortedFindings.length})</span></div>}>
      <SpaceBetween size="m">
